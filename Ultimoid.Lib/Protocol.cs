        ...
        public static Datagram Deserialize(byte[] networkData) {
            if (networkData.Length > MaxDatagramLength) {
                throw new ArgumentException(
                    $"Datagram longer than maximum allowed size (actual size: ${networkData.Length}",
                    nameof(networkData));
            }
            ...
            Array.Copy(networkData, MagicHeaderLength + SeqLength + AckLength + AckFieldLength, messageIdBytes, 0,
